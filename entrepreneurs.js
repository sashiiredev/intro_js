const entrepreneurs = [
    { first: 'Steve', last: 'Jobs', year: 1955 },
    { first: 'Oprah', last: 'Winfrey', year: 1954 },
    { first: 'Bill', last: 'Gates', year: 1955 },
    { first: 'Sheryl', last: 'Sandberg', year: 1969 },
    { first: 'Mark', last: 'Zuckerberg', year: 1984 },
    { first: 'Beyonce', last: 'Knowles', year: 1981 },
    { first: 'Jeff', last: 'Bezos', year: 1964 },
    { first: 'Diane', last: 'Hendricks', year: 1947 },
    { first: 'Elon', last: 'Musk', year: 1971 },
    { first: 'Marissa', last: 'Mayer', year: 1975 },
    { first: 'Walt', last: 'Disney', year: 1901 },
    { first: 'Larry', last: 'Page', year: 1973 },
    { first: 'Jack', last: 'Dorsey', year: 1976 },
    { first: 'Evan', last: 'Spiegel', year: 1990 },
    { first: 'Brian', last: 'Chesky', year: 1981 },
    { first: 'Travis', last: 'Kalanick', year: 1976 },
    { first: 'Marc', last: 'Andreessen', year: 1971 },
    { first: 'Peter', last: 'Thiel', year: 1967 }
  ];

  // 1er questions

  const result = entrepreneurs.filter(entrepreneurs => entrepreneurs.year > 1969 && entrepreneurs.year < 1980);
  console.log("Listes des entrepreneurs qui sont nés dans les années 70");
  console.log(result)

// 2nd questions
array1 = [];

entrepreneurs.forEach(element => 
    array1 = console.log(element.first + " "+ element.last)
);

// 3eme questions
entrepreneurs.forEach(element => 
     console.log(2019 - element.year )
);

//Les entrepreneurs par ordre alphabetique
function alphabetique(){
    entrepreneurs.sort.year
  }
  function trie(a,b) {
    if (a.last < b.last)
      return -1;
    if (a.last > b.last)
      return 1;
    return 0;
  }
  // Fonction qui fait le trie
  function alphabeticallyOrder(){
    console.log(entrepreneurs.sort(trie));
  }
  alphabeticallyOrder();


